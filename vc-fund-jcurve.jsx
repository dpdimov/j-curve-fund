...
const VCFundJCurve = () => {
  ...
  const stageProfiles = {
    seed: {
      ...
      followOnToB: 4000,
      survivalToA: 0.40,
      survivalToB: 0.65, // of those who reach A
      survivalToExit: 0.70, // of those who reach B
      timeToA: 18, // months
      timeToB: 36, // months from investment
      timeToExit: 84, // months from investment
      exitDistribution: [
  ...
  // Deterministic simulation using expected values
  const simulateFund = (p) => {
    const monthlyFlows = new Array(180).fill(0); // 15 years
    const monthlyCallsOut = new Array(180).fill(0);
    const monthlyDistributions = new Array(180).fill(0);
    
    // Allocate companies to stages
    ...
    // Investment pacing: spread over deployment years
    ...
    stages.forEach(stage => {
      ...
      for (let i = 0; i < numInStage; i++) {
        ...
        // Track expected value through stages using survival rates
        let expectedCompanies = 1.0;
        let totalCapitalInCompany = initialInvestment;

        // Follow-on to Series A (if starting from seed)
        ...
        // Calculate expected exit value using distribution
    ...
    // Calculate metrics at various points
  ...
  const maxMonth = 156; // Show 13 years
            ...
            <div style={{
              ...
              <svg width={width} height={height} style={{ display: 'block' }}>
                ...
                {showComparison && comparisonData && (
                  <>
                    <path
                      d={generateAreaPath(comparisonData.cumulativeFlows)}
                      fill="url(#vcComparisonGradient)"
                    />
                    <path
                      d={generatePath(comparisonData.cumulativeFlows)}
                      fill="none"
                      stroke="#bf616a"
                      strokeWidth="2"
                      opacity="0.6"
                ...
                {/* Main curve area */}
                <path
                  d={generateAreaPath(fundData.cumulativeFlows)}
                  fill="url(#vcCurveGradient)"
                />

                {/* Main curve */}
                <path
                  d={generatePath(fundData.cumulativeFlows)}
                  fill="none"
                  stroke="url(#vcLineGradient)"
                  strokeWidth="2.5"
                  strokeLinecap="round"
